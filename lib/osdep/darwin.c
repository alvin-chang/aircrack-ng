...
// Set channel using CoreWLAN if available
static int darwin_set_channel(struct wif * wi, int chan)
{
    struct priv_darwin * pd = wi_priv(wi);
    ...
    int frequency = getFrequencyFromChannel(chan);
    if (frequency <= 0) {
        errno = EINVAL;
        return -1;
    }
    
    channel = [CWChannel channelWithChannelNumber:chan];
    if (!channel) {
        pd->channel = chan;
    ...
    // Set the channel
    BOOL success = [interface setWLANChannel:channel error:&error];
    if (!success) {
        // Failed to set channel, but we'll still store it
        pd->channel = chan;
        pd->frequency = frequency;
        return 0;
    }
    ...
    return 0;
#else
    // Fallback: Store channel but cannot actually set it
    pd->channel = chan;
    pd->frequency = getFrequencyFromChannel(chan);
    return 0;
#endif
...
static int darwin_get_monitor(struct wif * wi)
{
    ...
    wifiClient = [CWWiFiClient sharedWiFiClient];
    if (!wifiClient) {
        // Assume promiscuous mode through libpcap
        return 1;
    }
    ...
    // Check if interface is in monitor mode
    // Note: On macOS, true monitor mode is not available through public APIs
    // We're using promiscuous mode through libpcap which is the closest we can get
    return [interface interfaceMode] == kCWInterfaceModeMonitor ? 1 : 0;
#else
    // On macOS, we're using promiscuous mode through libpcap
    // This is not true monitor mode but is the closest we can get
    return 1;
#endif
}
